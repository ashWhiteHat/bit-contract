...
トランザクションの一般形は第4章（Formalization）の $tx=(\mathsf{in}, \mathsf{out}, \mathsf{aux})$ に従う。以降は各“型”ごとに、$\mathsf{out}/\mathsf{aux}$ に課す追加制約（marker、含めるパラメータ、$\mathsf{c}_{id}$ 導出など）のみを列挙する。
