...
焦点ケース（IOM-CE）の状態機械（$S0, S0D, S0X, S1, S2$）と各遷移ラベル $\ell$ について、テンプレート $\tau$ とインスタンス $\mathsf{cid}$ を固定すれば、任意の $\Sigma$ と $tx$ に対して $\mathsf{Pred}_{\tau,\ell}(\Sigma, tx, \mathsf{cid})$ の真偽が一意に定まり、真である場合に限り $\mathsf{out}(tx) \in \mathsf{TransSpec}_{\tau,\ell}(\Sigma, \mathsf{cid})$ が成立する。
...
$\mathrm{Disc}(\mathsf{Operator})=\varnothing$。すなわち、運営 $C$ の単独署名だけで $S1/S2$ に到達する admissible な遷移は存在しない。

\paragraph{命題 P2'（裁量の移送）}
運営が関与する仲裁遷移は存在するが、それらは常に $(A \text{ or } B)$ の署名を伴うため、裁量は「運営単独」から「当事者の同意の証跡」へ移送される。
...
期限条件（$\operatorname{after}(d), \operatorname{after}(t)$）により、ロック状態が永久残留しない（Finalize/Timeout が誰でも起動可能）。
