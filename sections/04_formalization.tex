...
\textbf{実装上は、$\mathsf{data}$ は script / witness / commitment / metadata に埋め込む抽象化であり、UTXO に独立の data 欄があることを仮定しない。}
...
\paragraph{遷移規則（状態更新）}
$tx$ が上の基礎妥当性を満たし、かつ admissible（Definition 10）であるとき、状態遷移
\begin{equation}
...
\begin{equation}
	\tau := (\mathsf{tid}, \mathsf{ArgSchema}, \mathsf{Logic}, \mathsf{TransSpec})
\end{equation}
\begin{itemize}
...
\end{equation}
で表す。$\vec{a}=(a_1,\dots,a_k)$ はテンプレートが定める初期化引数列、$\mathsf{participants}$ は（必要なら）当事者集合（例：Buyer/Seller/Operator）である。
...
\end{equation}
と表す。ここで $u^\star$ の上付き $\star$ は lock 済み状態を示す記号である。
$\mathsf{stateTag}$ はテンプレートが定める有限個の状態ラベル（例：$S0, S0D, S0X$）である。
\textbf{実装上は、$\mathsf{stateTag}$ も script / witness / commitment / metadata に埋め込む抽象化であり、UTXO に独立の状態欄があることを仮定しない。}
...
\paragraph{Definition 9（Contract Create / Lock）}
\begin{itemize}
...
ここで $\ell$ はテンプレートが定める遷移ラベルである。型注釈として、
\begin{equation}
...
\paragraph{Definition 11（Finalize / Timeout）}
