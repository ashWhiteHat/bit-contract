...
\begin{itemize}
	\item $\Szero$: Funded escrow（資金拘束）：Buyerが資金をエスクローとして拘束
	\item $\SzD$: Delivered pending（配送通知済み・検収猶予中）：Sellerが発送の完了を通知
	\item $\SzX$: Disputed（紛争中：タイムアウトor仲裁のみ）：Buyerが発送の完了を認めず紛争
	\item $\Sone$: Release to Seller（支払）：資金がSellerへ解放
	\item $\Stwo$: Refund to Buyer（返金）：資金がBuyerへ返還
\end{itemize}
...
\begin{itemize}
	\item $d$：SellerConfirm後の“検収猶予”
	\item $t$：全体の最終期限（$t \ge d$ を仮定）
\end{itemize}

\begin{itemize}
	\item $\BuyerLock(\sigA, \Seller, p)$.
