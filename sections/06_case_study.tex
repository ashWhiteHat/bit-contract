...
\begin{itemize}
	...
	\item $S0 \to S0D$（$\mathrm{SellerDeliver}$、起動者：$\mathsf{addr}_B$）。
	\item $S0/S0D \to S1$（$\mathrm{BuyerConfirm}$、起動者：$\mathsf{addr}_A$）。
	\item $S0/S0D \to S1$（$\mathrm{FinalizeContract}$、$\operatorname{after}(d)$、起動者：Anyone）。
	...
	\item タイムアウトにより $S0/S0D/S0X/S0C \to S2$ が可能（$\mathrm{ContractTimeout}$、$\mathsf{timeout\_condition}(\mathsf{stateTag})=\mathsf{true}$、起動者：Anyone）。
\end{itemize}
...
\begin{table}[htbp]
	...
	\begin{tabular}{p{0.18\textwidth}p{0.14\textwidth}p{0.13\textwidth}p{0.17\textwidth}p{0.08\textwidth}p{0.20\textwidth}}
		\toprule
		遷移                                & 前提                                                                               & 起動者                                                   & 認証/条件                                          & 次状態   & 効果                                                                           \\
		\midrule
		$\mathrm{BuyerLock}$              & 初期（未ロック）                                                                         & $\mathsf{addr}_A$                                     & $\sigma_A$                                     & $S0$  & LockUtxo $p$（拘束開始）                                                           \\
		$\mathrm{SellerDeliver}$          & $S0$                                                                             & $\mathsf{addr}_B$                                     & $\sigma_B$                                     & $S0D$ & $\mathsf{stateTag}=S0D$ に更新（ロック継続）                                           \\
		$\mathrm{BuyerConfirm}$           & $S0/S0D$                                                                         & $\mathsf{addr}_A$                                     & $\sigma_A$                                     & $S1$  & $p(1-m-b)\to\mathsf{addr}_B$, $pm\to\mathsf{addr}_C$, $pb\to\mathsf{addr}_A$ \\
		$\mathrm{FinalizeContract}$       & $S0/S0D$, $\operatorname{after}(d)$                                              & Anyone                                                & --                                             & $S1$  & $p(1-m-b)\to\mathsf{addr}_B$, $pm\to\mathsf{addr}_C$, $pb\to X$              \\
		$\mathrm{BuyerDispute}$           & $S0D$                                                                            & $\mathsf{addr}_A$                                     & $\sigma_A$                                     & $S0X$ & $\mathsf{stateTag}=S0X$ に更新（ロック継続）                                           \\
		$\mathrm{OperatorMediateRelease}$ & $S0X$                                                                            & $\mathsf{addr}_C + (\mathsf{addr}_A/\mathsf{addr}_B)$ & $\sigma_C$ と $(\sigma_A \text{ or } \sigma_B)$ & $S1$  & $p(1-m)\to\mathsf{addr}_B$, $pm\to\mathsf{addr}_C$                           \\
		$\mathrm{OperatorMediateRefund}$  & $S0X$                                                                            & $\mathsf{addr}_C + (\mathsf{addr}_A/\mathsf{addr}_B)$ & $\sigma_C$ と $(\sigma_A \text{ or } \sigma_B)$ & $S2$  & $p\to\mathsf{addr}_A$                                                        \\
		$\mathrm{BuyerCancel}$            & $S0/S0D$                                                                         & $\mathsf{addr}_A$                                     & $\sigma_A$                                     & $S0C$ & $\mathsf{stateTag}=S0C$ に更新（ロック継続）                                           \\
		$\mathrm{SellerAcceptCancel}$     & $S0C$                                                                            & $\mathsf{addr}_B$                                     & $\sigma_B$                                     & $S2$  & $p\to\mathsf{addr}_A$                                                        \\
		$\mathrm{ContractTimeout}$        & $S0/S0D/S0X/S0C$, $\mathsf{timeout\_condition}(\mathsf{stateTag})=\mathsf{true}$ & Anyone                                                & --                                             & $S2$  & $p\to\mathsf{addr}_A$                                                        \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{center}
	\small
	\begin{tabular}{l}
		$\mathsf{addr}_A \rightarrow$ Escrow: LockUtxo $p$; $\mathrm{BuyerLock}$, state $S0$                                                \\
		...
		Escrow $\rightarrow$ $\mathsf{addr}_B$: $p(1-m-b)$ (BuyerConfirm/FinalizeContract)                                                  \\
		Escrow $\rightarrow$ $\mathsf{addr}_C$: $pm$ (BuyerConfirm/FinalizeContract)                                                        \\
		Escrow $\rightarrow$ $\mathsf{addr}_A$: $pb$ (BuyerConfirm)                                                                         \\
		Escrow $\rightarrow$ $X$: $pb$ (FinalizeContract)                                                                                   \\
		Escrow $\rightarrow$ $\mathsf{addr}_B$: $p(1-m)$ (OperatorMediateRelease)                                                           \\
		Escrow $\rightarrow$ $\mathsf{addr}_C$: $pm$ (OperatorMediateRelease)                                                               \\
		Escrow $\rightarrow$ $\mathsf{addr}_A$: $p$ (OperatorMediateRefund/ContractTimeout/SellerAcceptCancel)
	\end{tabular}
\end{center}
...
\begin{enumerate}
	...
	\item $\mathrm{BuyerConfirm}$：引数は $\sigma_A$、modifier は $S0$ または $S0D$、validation は $\mathsf{addr}_A$ の署名検証、post\_state は $S1$。FinalizeContract と同様の遷移で、bounty は $\mathsf{addr}_A$ に帰属する。
...
\begin{table}[htbp]
	...
	\begin{tabular}{p{0.14\textwidth}p{0.08\textwidth}p{0.19\textwidth}p{0.23\textwidth}p{0.12\textwidth}p{0.12\textwidth}}
		...
		$\mathrm{BuyerDispute}$           & $S0D$            & $\sigma_A$                                       & $\mathsf{stateTag}=S0X$ に更新（ロック継続）                                                             & $\mathsf{addr}_A$                                     & A, L    \\
		...
		$\mathrm{FinalizeContract}$       & $S0/S0D$         & $\operatorname{after}(d)$, 受取先 $X$               & $p(1-m-b)$ を $\mathsf{addr}_B$、$p\cdot m$ を $\mathsf{addr}_C$、$p\cdot b$ を $X$ へ               & Anyone                                                & S, L    \\
		$\mathrm{ContractTimeout}$        & $S0/S0D/S0X/S0C$ & $\mathsf{timeout\_condition}(\mathsf{stateTag})$ & $p$ を $\mathsf{addr}_A$ に返金（$S2$）                                                              & Anyone                                                & S, L    \\
		\bottomrule
	\end{tabular}
\end{table}
...
Finalization は期限条件により確定させる遷移であり、$\mathrm{FinalizeContract}$ と $\mathrm{ContractTimeout}$ を含む。いずれも誰でも起動可能で、$\operatorname{after}(d)$ により $S0/S0D \to S1$ を確定し、$\mathsf{timeout\_condition}(\mathsf{stateTag})=\mathsf{true}$ のとき $S0/S0D/S0X/S0C \to S2$ を確定する。
...
Liveness：決着性
$S0/S0D$は$\mathrm{BuyerConfirm}$か$\operatorname{after}(d)$の$\mathrm{FinalizeContract}$で$S1$へ、
$S0X$は仲裁（$\mathsf{addr}_C + (\mathsf{addr}_A \text{ or } \mathsf{addr}_B)$）か、最終的に$\mathsf{timeout\_condition}(\mathsf{stateTag})=\mathsf{true}$の$\mathrm{ContractTimeout}$で$S2$へ。
