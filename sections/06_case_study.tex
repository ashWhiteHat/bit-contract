...
\begin{transitions}
	...
	\tr{\Sone}{p\,\LUTXO(\cid,\stag\in\{\SzD,\SzX\})}{\to \pmarg{p}{m}\,\uB\ \text{and}\ \pfee{p}{m}\,\uC}
	\tr{\Stwo}{p\,\LUTXO(\cid,\stag\in\{\Szero,\SzD,\SzX\})}{\to p\,\uA}
\end{transitions}

\subsection{Template InitParams}
$\mathsf{InitParams}=(m, d, t)$
マージン率: $m$
配送期間：$d$
コントラクトタイムアウト: $t$
...
\begin{table}[htbp]
	...
	\begin{tabular}{p{0.14\textwidth}p{0.08\textwidth}p{0.19\textwidth}p{0.23\textwidth}p{0.12\textwidth}p{0.12\textwidth}}
		...
		$\Finalize$           & $\SzD$       & $\after(d)$                                         & $\pmarg{p}{m}$ を $\Seller$、$\pfee{p}{m}$ を $\Oper$ へ        & Anyone                   & S, L    \\
