...
買い手アドレスを addrA、販売者アドレスを addrB、運営者（仲介者）アドレスを addrC とし、買い手が価格 $p$ の商品購入を試み、運営者はマージン率 $m$ を設定する。
...
\begin{table}[t]
	...
	\begin{tabularx}{0.92\linewidth}{lX}
		...
		記号    & 意味                                                   \\
		\midrule
		addrA & 買い手（Buyer）アドレス                                       \\
		addrB & 販売者（Seller）アドレス                                      \\
		addrC & 運営者（Operator / Platform）アドレス                         \\
		$p$   & 商品価格                                                 \\
		$m$   & マージン率                                                \\
		$d$   & 配送期間（Delivered 判定の想定期間）                              \\
		$t$   & コントラクト・タイムアウト（$t \ge d$）                             \\
		$b$   & FinalizeContract 報酬率（bounty rate, $0 \le b \le 1-m$） \\
		\bottomrule
	\end{tabularx}
...
InitParams $=(m,d,t,b)$ は運営者 addrC が CreateContract（インスタンス作成）時に確定させるテンプレート固定パラメータであり、
以後の型検証およびテンプレート検証で参照される。
ここでは $0 \le b \le 1-m$ を仮定する。
また、addrdeployer $=$ addrC として cid を導出し、例えば
\[
	\mathrm{cid} := H(\mathrm{addrdeployer} \parallel \mathrm{tindex} \parallel \mathrm{InitParams})
\]
のように決定的に定める（同一入力から一意であることが本質）。

\paragraph{LockParams.}
LockParams $=(\mathrm{addrB},p)$ は買い手 addrA が BuyerLock により資金拘束（LockUtxo）を開始する際に与える。
販売者アドレス addrB と価格 $p$ を指定し、以降の支払い先・返金先の整合性検証に用いる。
...
stateTag は Locked UTXO に付随する唯一の共有状態語彙として扱う。
...
\begin{table*}[t]
	...
	\begin{tabularx}{\textwidth}{l l l l l X}
		...
		$\sigma_A$                                                     &
		$S0$                                                           &
		$p$ を拘束し $\mathrm{UTXO}^{\star}(\mathrm{cid},\mathrm{stateTag}{=}S0)$ を生成                                                \\
		...
		$\sigma_A$                                                     &
		$S1$                                                           &
		$p(1-m-b)\rightarrow \mathrm{addrB},\; pm\rightarrow \mathrm{addrC},\; pb\rightarrow \mathrm{addrA}$（bounty は Buyer に帰属） \\
		...
		$S1$                                                           &
		$p(1-m-b)\rightarrow \mathrm{addrB},\; pm\rightarrow \mathrm{addrC},\; pb\rightarrow \mathrm{addrF}$（bounty は起動者に帰属）     \\
		...
		$S0X$                                                          &
		$\mathrm{addrC}+(\mathrm{addrA}/\mathrm{addrB})$               &
		$\sigma_C$ と $(\sigma_A\ \mathrm{or}\ \sigma_B)$               &
		$S1$                                                           &
		$p(1-m)\rightarrow \mathrm{addrB},\; pm\rightarrow \mathrm{addrC}$                                                       \\
...
第4章の定義（テンプレート $\tau=(\mathrm{tindex},\mathrm{InitParams},\mathrm{LockParams},\mathrm{StateSet},\mathrm{LogicSet},\mathrm{TransSpec})$）に従う。
...
本テンプレートは Disc(Operator)$=\emptyset$ を満たす：運営者 addrC は単独で $S1/S2$ に到達できない。
...
さらに FinalizeContract は誰でも実行可能で bounty（$pb$）が起動者 addrF に帰属するため、期限到来後に実行されやすい。
